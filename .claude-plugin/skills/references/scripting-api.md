...
name: scripting-api
description: >-
  Reference for API Check scripting: HTTP request steps, selectors, variables,
  assertions, and authentication. Used by monitoring-setup and check-management
  skills.
---

# API Check scripting reference

API checks execute multi-step HTTP request sequences with assertions. Scripts
are JSON arrays of step objects executed sequentially. If any assertion fails,
execution stops and an alert is raised.

## Table of contents

- [Step structure](#step-structure)
- [HTTP request steps](#http-request-steps)
- [Settings and authentication](#settings-and-authentication)
- [Variables](#variables)
...
```json
[
  { "step_def": "C_GET", "values": { "url": "https://api.example.com" } },
  { "step_def": "V_HTTP_STATUS_CODE_SUCCESSFUL" }
]
```

Each step has `step_def` (type identifier) and optional `values` (arguments).
...
| `files` | File uploads (map of field name → file URL) |
...
Configures defaults applied to all subsequent requests. Optional — place first
if used.
...
**Bearer token** — use a request step to authenticate, extract token with
`C_SET_VARIABLE_SELECTOR`, then use `$TOKEN$` in headers of subsequent requests.

**Client certificate (mTLS)** — set `certificate`, `key`, and optionally
...
Set variables during execution and reference them in later steps via
`$VARIABLE_NAME$` syntax. Names are case-insensitive (stored uppercase).
Unresolved variables remain literal.
...
The `regex` and `group` parameters on selector/header variable setters allow
substring extraction:

- `regex` — Go regex pattern
- `group` — `0` for full match, `1+` for capture groups, or named group from
  `(?P<name>...)` pattern
...
All selector assertions take a `selector` parameter — see [Selectors](#selectors).

## Selectors

Selectors extract values from response bodies. The format depends on content
type (auto-detected from `Content-Type` header):
